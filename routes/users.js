...
router.post("/init", function(req, res, next) {
  ...
  var ustatus = 1;
  var uname = dyncname.randomNickName();
    ...
    dynchttp.validateUser(req, res, function(err, userid){
        if (userid) {
            ...
            if (null == nickname || nickname.length == 0) {
                return dynchttp.sendMissParams(req, res, "nickname");
            }
            db.updateNickname([nickname, userid], function (err, response) {
                if (!err) {
                    if(response.affectedRows == 1) {
                        var responseJson = {
                            requestid: req._startTime.valueOf(),
                        ...
                        var responseJson = {
                            requestid: req._startTime.valueOf(),
                            code: 400,
                            message: 'update user nickname failed'
                        };
                        dynchttp.sendSuccess(req, res, responseJson);
                    }
                } else {
